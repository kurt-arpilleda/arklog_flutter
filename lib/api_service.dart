...
class ApiService {
  ...
  Future<String?> fetchQRCode() async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_fetchQRCode.php");
          final response = await httpClient.get(uri);

          if (response.statusCode == 200) {
            final data = jsonDecode(response.body);
            if (data["success"] == true) {
              return _ApiResult(data["code"] as String?, apiUrl);
            }
            throw Exception(data["error"] ?? "Failed to fetch QR code");
          }
          throw Exception("HTTP ${response.statusCode}");
        });
  ...
  Future<bool> insertIdNumberQR(String idNumber) async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_insertIdNumberQR.php");
          final response = await httpClient.post(
            uri,
            body: {'idNumber': idNumber},
          );
          ...
          if (response.statusCode == 200) {
            ...
            throw Exception(data["error"] ?? "Failed to insert idNumber into QR");
          }
          throw Exception("HTTP ${response.statusCode}");
        });
  ...
  Future<Map<String, dynamic>> checkPendingConfirmation() async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_checkPendingConfirmation.php");
          final response = await httpClient.get(uri);
          ...
          if (response.statusCode == 200) {
            ...
            throw Exception(data["message"] ?? "Failed to check pending confirmation");
          }
          throw Exception("HTTP ${response.statusCode}");
        });
  ...
  Future<Map<String, dynamic>> fetchTodos(String idNumber) async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_fetchTodo.php?idNumber=$idNumber");
          final response = await httpClient.get(uri);
          ...
          if (response.statusCode == 200) {
            ...
            if (data["success"] == true) {
              ...
              throw Exception(data["error"] ?? "Failed to fetch todos");
            }
          }
          throw Exception("HTTP ${response.statusCode}");
        });
  ...
  Future<Map<String, dynamic>> updateTodo(int todoId, int done) async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_updateTodo.php");
          final response = await httpClient.post(
            uri,
            body: {
  ...
  Future<Map<String, dynamic>> fetchTodoCount(String idNumber) async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_fetchTodoCount.php?idNumber=$idNumber");
          final response = await httpClient.get(uri);
          ...
          if (response.statusCode == 200) {
            ...
            if (data["success"] == true) {
              ...
              throw Exception(data["error"] ?? "Failed to fetch todo count");
            }
          }
          throw Exception("HTTP ${response.statusCode}");
        });
  ...
  Future<Map<String, dynamic>> addTodo(String idNumber, String task, [String? appToOpen]) async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_addTodo.php");
          final response = await httpClient.post(
            uri,
            body: {
              'idNumber': idNumber,
  ...
  Future<Map<String, dynamic>> deleteTodos(List<int> todoIds) async {
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        final result = await _makeParallelRequest((apiUrl) async {
          final uri = Uri.parse("${apiUrl}V4/Others/Kurt/ArkLogAPI/kurt_deleteTodo.php");
          final response = await httpClient.post(
            uri,
            body: {
              'todoIds': jsonEncode(todoIds),
            },
          );
